...
void houghTrans::getPoint()
{
	...
	//end
	//cout << "cluster 1 end" << endl;
	//cout << "judge point on line start" << endl;//this cost about 30seconds for a 3000*4000pixels picture ,need to be reduced;
	//cout << thetav.size() << endl;
	...
	//cout << "judge point on line finished " << endl;
	boolean *clustered = new boolean[pointx.size()];
	for (int i = 0; i < pointx.size(); i++) {
		clustered[i] = false;
	}
	vector<int> resx, resy;
	//cout << "cluster 2 start" << endl;
	for (int i = 0; i < pointx.size(); i++) {
	...
	//cout << "cluster 2 finished" << endl;
	pointx = resx;
	pointy = resy;
	//cout << "pointx.size" << pointx.size();
	//cout << "get point called return;" << endl;
}
